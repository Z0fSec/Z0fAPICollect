...
public class APICheckUI extends Component implements UIHandler, IMessageEditorController, IHttpListener {
    ...
    private void setupData() {
        ...
        saveAPIDataButton.addActionListener(e -> {
            ...
            ExcelWriter writer = ExcelUtil.getWriter("D:\\Z0fData\\ZAC-API接口收集-" + System.currentTimeMillis() + ".xlsx", "API收集集合");
            // 添加标题，根据需要添加别名
            writer.addHeaderAlias("column0", "序号");
            writer.addHeaderAlias("column1", "请求方法");
            writer.addHeaderAlias("column2", "主机");
            writer.addHeaderAlias("column3", "路径");
            writer.addHeaderAlias("column4", "URL");
            writer.addHeaderAlias("column5", "请求体");
            writer.addHeaderAlias("column6", "响应体");
            List<Map<String, Object>> data = new ArrayList<>();
            for (int i = 0; i < urldata.size(); i++) {
                Map<String, Object> row1 = new HashMap<>();
                row1.put("column0", urldata.get(i).getId());
                row1.put("column1", urldata.get(i).getMethod());
                row1.put("column2", urldata.get(i).getHost());
                row1.put("column3", urldata.get(i).getPath());
                row1.put("column4", urldata.get(i).getUrl());
                String apiRequest = new String(urldata.get(i).getRequestResponse().getRequest(), StandardCharsets.UTF_8);
                if (apiRequest.length() > 32767) {
                    apiRequest = apiRequest.substring(0, 32767);
                }
                row1.put("column5", apiRequest);
                String apiResponse = new String(urldata.get(i).getRequestResponse().getResponse(), StandardCharsets.UTF_8);
                if (apiResponse.length() > 32767) {
                    apiResponse = apiResponse.substring(0, 32767);
                }
                row1.put("column6", apiResponse);
                data.add(row1);
            }
            // 写入数据到CSV文件
